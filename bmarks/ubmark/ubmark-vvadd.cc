...
// ubmark-vvadd
//========================================================================
// This is a simple micro-benchmark which does vector-vector add. It
// can generate its own dataset which is statically included as part of
// the binary. To generate a new dataset simply use the -dump-dataset
// command line option, place the new dataset in the subproject
// directory, and make sure you correctly inlude the dataset.

#include "pinfo.h"

#include "ubmark-vvadd-scalar.h"
...
void vvadd_scalar_unopt( int dest[], int src0[],
                         int src1[], int size )
{
...
void dump_dataset( const std::string& fname, int size )
{
  ...
  vvadd_scalar( &dest[0], &src0[0], &src1[0], size );

  // Output dataset

  stdx::DefaultStdoutFstream fout(fname);

  fout << "// Data set for ubmark-vvadd\n\n";
...
{
  { "scalar",       &vvadd_scalar        },
  { "scalar-unopt", &vvadd_scalar_unopt  },
  { "",             0                    },
};
