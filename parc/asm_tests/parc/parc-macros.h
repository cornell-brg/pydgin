...
//------------------------------------------------------------------------
// TEST_AMO : Helper macros for atomic arithmetic instructions
//------------------------------------------------------------------------

#define TEST_AMO_OP( inst_, base_, src_, result0_, result1_ )           \
    la    $2, base_;                                                    \
    li    $3, src_;                                                     \
    inst_ $4, $2, $3;                                                   \
    lw    $5, 0($2);                                                    \
    TEST_CHECK_EQ( $4, result0_ );                                      \
    TEST_CHECK_EQ( $5, result1_ );                                      \

#define TEST_AMO_DEST_BYP( nops_, inst_, base_, src_,                   \
                           result0_, result1_ )                         \
    la    $2, base_;                                                    \
    li    $3, src_;                                                     \
    inst_ $4, $2, $3;                                                   \
    TEST_INSERT_NOPS( nops_ );                                          \
    addiu $6, $4, 0;                                                    \
    TEST_CHECK_EQ( $6, result0_ );                                      \
    lw    $5, 0($2);                                                    \
    TEST_CHECK_EQ( $5, result1_ );                                      \

#define TEST_AMO_SRC01_BYP( src0_nops_, src1_nops_,                     \
                            inst_, base_, src_, result0_, result1_ )    \
    la    $2, base_;                                                    \
    TEST_INSERT_NOPS( src0_nops_ );                                     \
    li    $3, src_;                                                     \
    TEST_INSERT_NOPS( src1_nops_ );                                     \
...
#define TEST_AMO_SRC10_BYP( src1_nops_, src0_nops_,                     \
                            inst_, base_, src_, result0_, result1_ )    \
    li    $3, src_;                                                     \
    TEST_INSERT_NOPS( src1_nops_ );                                     \
    la    $2, base_;                                                    \
