...
class ParcSim( Sim ):
  ...
  def run( self ):
    ...
    print "Executed %d pcalls" % self.states.xpc_stats.count
    
    # Get executable name
    exe_filename = path_leaf(self.states.exe_name)
    ...
    csvOutput.write("pcall_id,size,total_insts,pcall_insts,ctrl_cond,ctrl_uncond,jal,int_arith,llfu_arith,ld,st,amo_movs,amo_arith,syscalls,eret,mov,nop")
    # function calls
    funcOutput.write("pcall_id,(call_pc, func_pc)")
    # ctrlOutput
    divOutput.write("pcall_id,xi,(branch_pc, target_pc)")
    divTOutput.write("%d" % (self.states.xpc_stats.count))
    # Number of dynamic instructions
    instOutput.write("pcall_id,number_of_instructions")
    for i, pcall in enumerate(self.states.xpc_stats.pcalls):
      ...
      total_xpc_stats.insts.ctrl.uncond += pcall.insts.ctrl.uncond
      ...
      # Write to file
      csvOutput.write("\n%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d" %(i, pcall.size, self.states.xpc_stats.insts_t[i] if i < len(self.states.xpc_stats.insts_t) else 0, pcall.insts.count, pcall.insts.ctrl.cond, pcall.insts.ctrl.uncond, pcall.insts.ctrl.jal, pcall.insts.arith.ints, pcall.insts.arith.llfu, pcall.insts.mem.ld, pcall.insts.mem.st, pcall.insts.amo.mov, pcall.insts.amo.arith, pcall.insts.sys.calls, pcall.insts.sys.ret, pcall.insts.misc.mov, pcall.insts.misc.nop))
      ...
      print "      Mix:"
      print "        Branches:"
      print "          Total          = %d" % (pcall.insts.ctrl.cond + pcall.insts.ctrl.uncond)
    ...
    # Total
    csvOutput.write("\n%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d" %(-1, total_xpc_stats.size, self.states.num_insts, total_xpc_stats.insts.count, total_xpc_stats.insts.ctrl.cond, total_xpc_stats.insts.ctrl.uncond, total_xpc_stats.insts.ctrl.jal, total_xpc_stats.insts.arith.ints, total_xpc_stats.insts.arith.llfu, total_xpc_stats.insts.mem.ld, total_xpc_stats.insts.mem.st, total_xpc_stats.insts.amo.mov, total_xpc_stats.insts.amo.arith, total_xpc_stats.insts.sys.calls, total_xpc_stats.insts.sys.ret, total_xpc_stats.insts.misc.mov, total_xpc_stats.insts.misc.nop))
    # Close file
    csvOutput.close()
    funcOutput.close()
    divOutput.close()
    ...
    # Get to it
    print "  count = %d" % self.states.xpc_stats.count
